...
6. **Scorecard** — `renderVerdict()` renders into `#verdict` inside the `#statsModal` popup (opened from the header bar-chart icon; not on the page itself): 
...
- Visual order is generator panel → ad → corpus stats. DOM order puts the corpus aside (`#corpusSection`) before the stage; `.lab{order:2}` pushes it below. Generating auto-scrolls to the ad (`showAd()`). Smooth scrolling is CSS (`html{scroll-behavior:smooth}`), disabled under `prefers-reduced-motion`.
- The header is sticky, with three icon buttons: book scrolls to `#corpusSection`, restart arrow returns to top, bar chart opens the scorecard popup (`#statsModal` — closes on Close, backdrop tap, or Escape). Two traps here: `html`/`body` must use `overflow-x:clip`, not `hidden` (hidden creates scroll containers and silently kills `position:sticky`), and scroll targets need `scroll-margin-top` of the measured header height (`--headH`, set by `setHeadH()`; the header wraps taller on narrow screens, so it cannot be a CSS constant).
- Range inputs have custom track/thumb CSS (24px thumbs, 36px hit area) because default thumbs were too small on touch. Length is one dual-thumb slider: two overlaid `<input type=range>` sharing the 4–12 scale in `.dual`, `pointer-events:none` on the inputs and `auto` on the thumbs; `syncLen()` clamps whichever thumb moved, paints the fill, and z-index-swaps so stacked thumbs stay grabbable at both ends. Delivery is a `<select>`; "random" follows the Seriousness slider.
